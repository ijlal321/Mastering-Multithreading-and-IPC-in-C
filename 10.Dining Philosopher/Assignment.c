...
bool philosopher_get_access_both_spoons(phil_t *phil)
{
    ...
    while (true)
    {
        ...
        pthread_mutex_lock(&left_spoon->mutex); // got access to left
        left_spoon->is_used = true;
        left_spoon->phil = phil;

        int ret = pthread_mutex_trylock(&right_spoon->mutex);
        if (ret == 0)
        {
            right_spoon->is_used = true;
            right_spoon->phil = phil;
            // both left and right spoon acquired.
            return true;
        }
        else
        { // cannot get lock. + Also need to do error andling here later.
            left_spoon->is_used = false;
            left_spoon->phil = NULL;
            pthread_mutex_unlock(&left_spoon->mutex);
            sleep(1);
