...
pthread_cond_t queue_cond_Prod = PTHREAD_COND_INITIALIZER;
pthread_cond_t queue_cond_Cons = PTHREAD_COND_INITIALIZER;

static void *
prod_fn(void *arg) {

	const char *th_name = (char *)arg;
    printf("Producer Th %s start\n", th_name);

    while (1){  // always run
        pthread_mutex_lock(&queue_mutex);
        printf("Producer Th %s got lock\n", th_name);
        while (is_queue_full(Q)){
            printf("Producer Th %s waiting for queue to get not full.\n", th_name);
            pthread_cond_wait(&queue_cond_Prod, &queue_mutex);
        }
        printf("Producer Th %s start writing\n", th_name);
        while(!is_queue_full(Q)){
        ...
        print_Queue(Q);
        pthread_cond_broadcast(&queue_cond_Cons);
...
cons_fn(void *arg) {
	...
	const char *th_name = (char *)arg;
    printf("Consumer Thread %s start\n", th_name);

	while(1){
        pthread_mutex_lock(&queue_mutex);
        printf("Consumer Thread %s got lock\n", th_name);
        while (is_queue_empty(Q)){
            printf("Consumer Thread %s waiting for queue to get non-empty.\n", th_name);
            pthread_cond_wait(&queue_cond_Cons, &queue_mutex);
            printf("Consumer Thread %s Got the lock.\n", th_name);
        }
        printf("Consumer Thread %s start writing\n", th_name);
        while(!is_queue_empty(Q)){
            int * last_data = (int *)(deque(Q));
            free(last_data);
        }
        print_Queue(Q);
        pthread_cond_broadcast(&queue_cond_Prod);
        pthread_mutex_unlock(&queue_mutex);
...
main(int argc, char **argv) {
    ...
    pthread_cond_destroy(&queue_cond_Cons);
    pthread_cond_destroy(&queue_cond_Prod);
